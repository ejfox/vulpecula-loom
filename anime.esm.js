...
const globalVersions = { version: '4.0.0-beta.106', engine: null };
...
const render = (tickable, time, muteCallbacks, internalRender, tickMode) => {
 ...
 if (hasBegun && tickable._currentIteration !== _currentIteration) {
    ...
    if (_hasChildren) {
      forEachChildren(tickable, (/** @type {JSAnimation} */child) => child.reset(), true);
      const lastTLChild = /** @type {JSAnimation} */(/** @type {Timeline} */(tickable)._tail);
      if (lastTLChild) {
...
const tick = (tickable, time, muteCallbacks, internalRender, tickMode) => {
  ...
  if (tickable._hasChildren) {
    ...
    forEachChildren(tickable, (/** @type {JSAnimation} */child) => {
      hasRendered += render(
        child,
...
function addTlChild(childParams, tl, parsedTLPosition, targets, index, length) {
  ...
  const tlChild = targets ?
    new JSAnimation(targets,/** @type {AnimationParams} */(childParams), tl, TLPosition, false, index, length) :
