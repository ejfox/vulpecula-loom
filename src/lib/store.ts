...
export function useStore() {
  async function get<K extends keyof StoreSchema>(key: K): Promise<StoreSchema[K]> {
    return window.electron.ipcRenderer.invoke('store-get', key)
  }

  async function set<K extends keyof StoreSchema>(key: K, value: StoreSchema[K]): Promise<boolean> {
