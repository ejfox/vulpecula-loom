...
import type { ChatMessage as OpenRouterMessage } from './useOpenRouter'
import { useOpenRouter } from './useOpenRouter'
import { useSupabase } from './useSupabase'
import type { ChatHistory } from './useSupabase'
import type { TokenUsage } from './useOpenRouter'
...
export function useAIChat() {
  const { chat, hasValidKey: hasValidOpenRouterKey, MODEL_CONFIGS } = useOpenRouter()
  const { saveChatHistory, updateChatHistory, isConfigured: hasValidSupabaseConfig } = useSupabase()
  ...
  const modelName = computed(() => 
    MODEL_CONFIGS[currentModel.value]?.name ?? 'Unknown Model'
  )
  ...
  async function sendMessage(content: string) {
    ...
    try {
      ...
      const response = await chat(formattedMessages.value, {
        model: currentModel.value,
  ...
  async function saveToSupabase(assistantMessage: AIMessage) {
    ...
    try {
      ...
      console.log('Saving messages with token data:', formattedMessagesWithMetadata)
  ...
  async function loadChat(chatId: string) {
    ...
    try {
      ...
      messages.value = history.messages.map((msg: StoredMessage) => {
        ...
        const cost = msg.cost || calculateCost({
  ...
  function generateMarkdown(): string {
    ...
    lines.push(`stats:`)
    lines.push(`  total_tokens: ${chatStats.value.promptTokens + chatStats.value.completionTokens}`)
  ...
  async function exportChat() {
    try {
      ...
      const result = await window.electron.ipcRenderer.invoke('export-markdown', markdown)
      if (result.success) {
